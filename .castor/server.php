...
#[AsTask(description: 'Builds and starts the infrastructure, then install the server (composer, yarn, ...)')]
function start(): void
{
...
#[AsTask(description: 'Installs the server (composer, yarn, ...)')]
function install(): void
{
    io()->title('Installing the server');

    $basePath = \sprintf('%s/server', variable('root_dir'));
...
#[AsTask(description: 'Migrates database schema', namespace: 'server:db', aliases: ['migrate'])]
...
#[AsTask(namespace: 'server:dev', description: 'Start all messenger consumers', aliases: ['dev:consume'])]
...
{
    docker_compose_run('bin/console hivecache:admin:create');
}
